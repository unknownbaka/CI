...
# Build environment
apt-get update -qq && apt-get install --no-install-recommends -y bc binutils binutils-aarch64-linux-gnu binutils-arm-linux-gnueabi bison flex gcc libssl-dev make patch subversion ca-certificates curl git tar unzip wget zip zstd
